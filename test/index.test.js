const ketargs = require('../src/index').default

describe('ketargs', () => {
  it('should parse simple key-value pairs', () => {
    const args = { i: 'images' };
    expect(ketargs(args)).toEqual(['-i', 'images']);
  })

  it('should parse simple key-value pairs with array', () => {
    const args = { i: ['images', 'text']};
    expect(ketargs(args)).toEqual(['-i', 'images', 'text']);
  })

  it('should parse nested objects', () => {
    const args = { foo: { a: { b: 1, c: 2 } } };
  ...
  it('should support multiple keys', () => {
    ...
    expect(ketargs(args)).toEqual(['-i', 'images', '--size', '1920x1080', '--format', 'jpeg'])
  })
  ...
  it('should handle complex nested structures', () => {
    ...
    console.log(ketargs(args))
    expect(ketargs(args)).toEqual(
      ['--config', 'database.host=localhost', 'database.port=5432', 'server.enabled=true', 'server.port=8080', '-i', 'images', 'text', '-u', 'images', '--foo', 'a.b=1', 'a.c=2']
     );
  })
})
